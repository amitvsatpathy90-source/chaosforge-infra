...
variable "gateway_ingress_cidr" {
  description = "CIDR allowed to reach the edge-gateway's public IP on 8080 when enable_alb = false. Must be your own address, e.g. \"203.0.113.7/32\"."
  type        = string
  default     = null
...
resource "aws_vpc_security_group_ingress_rule" "edge_gateway_public" {
  ...
  to_port           = 8080
  ip_protocol       = "tcp"
  description       = "Direct public ingress in ALB-less mode — plain HTTP, so scoped to the operator's own CIDR"
}
...
resource "aws_vpc_security_group_ingress_rule" "cp_from_gateway" {
  ...
  to_port                      = 8081
  ip_protocol                  = "tcp"
  description                  = "Gateway -> CP, public JWT path"
}
...
resource "aws_vpc_security_group_ingress_rule" "cp_from_exec" {
  ...
  to_port                      = 8081
  ip_protocol                  = "tcp"
  description                  = "Exec -> CP, /internal path (mTLS; exec-cert-CN restricted in-app per ADR-0532)"
}
...
resource "aws_vpc_security_group_egress_rule" "exec_to_rpe_detection_actuator" {
  ...
  to_port                      = 8080
  ip_protocol                  = "tcp"
  description                  = "ChaosForge execution-service -> RPE detection actuator (steady-state probe only, C20)"
}
