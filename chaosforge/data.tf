# Unlike the `backend` block, `terraform_remote_state` is an ordinary data
# source — it CAN take variables. Supply state_bucket_name via
# terraform.tfvars (gitignored, see terraform.tfvars.example) rather than
# hardcoding the account-specific bucket name here.
variable "state_bucket_name" {
  description = "Bucket created by bootstrap/ (its state_bucket_name output)"
  type        = string
}
...
locals {
  ...
  private_subnet_ids                 = data.terraform_remote_state.foundation.outputs.private_chaosforge_subnet_ids
